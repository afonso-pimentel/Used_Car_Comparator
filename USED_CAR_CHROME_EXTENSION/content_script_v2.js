...
async function loadWebsiteParameters() {
  try {
    const websiteParameters = await fetchJsonFile(
      "websiteParameters/websiteParameters_v2.json"
    );
...
async function extractCarInformation() {
  ...
  async function extractDataForCurrentWebsite(websiteParams) {
    function getElementByDtIdentifier(params) {
      if (params.identifier) {
        const parentElement = document.querySelector(params.parent);
        const dts = parentElement.querySelector("dl").querySelectorAll("dt");
        for (const dt of dts) {
          if (dt.textContent.includes(params.identifier)) {
            return dt.nextElementSibling; // Gets the following dd element
          }
        }
      } else {
        return document.querySelector(params.parent);
      }
    }

    const carPriceElement = document.querySelector(
      websiteParams.carPriceElement.parent.includes("#")
        ? websiteParams.carPriceElement.parent
        : `[data-testid=${websiteParams.carPriceElement.parent}]`
    );
    console.log(carPriceElement);

    const carYearElement = getElementByDtIdentifier(
      websiteParams.carYearElement
    );
    console.log(carYearElement);

    const carEngineElement = getElementByDtIdentifier(
      websiteParams.carEngineElement
    );
    console.log(carEngineElement);

    const carEmissionsElement = getElementByDtIdentifier(
      websiteParams.carEmissionsElement
    );
    console.log(carEmissionsElement);

    const carFuelElement = getElementByDtIdentifier(
      websiteParams.carFuelElement
    );
    console.log(carFuelElement);

    const carSeatsElement = getElementByDtIdentifier(
      websiteParams.carSeatsElement
    );
    console.log(carSeatsElement);
    ...
    if (missingElements.length != 0) {
      ...
      const engineDisplacementString = carEngineDisplacement.replace(/\s/g, ""); // Remove spaces
      const engineDisplacementMatch =
        engineDisplacementString.match(/\d+(\.\d+)?/); // Match the numeric value
      ...
      const carPriceValue = parsePrice(carPrice);
      const carTotalPrice = carPriceValue + isvWithDiscount;
